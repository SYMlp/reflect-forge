...
| 锻造秘籍 | 反思时学的外部资料/前辈经验 | **只读参考**：被 cite 引用、滋养锻造，自己不进炉；本期烙印（锁定态），v2 接导师 agent |
| 外来的剑 | 别人的 SKILL.md（开源 / 他人锻的） | 借来用、融你的铁重锻成个人版；落在兵器架「外来名剑」区。**能被重锻的是剑，不是秘籍** |
| 探险 / 斩活 | 一类类工作 / 完成的工作 | |
...
**追加 2026-08-29 · v2 锻造谱系（增量字段，旧契约一个没动）**

```
GET  /api/armory    → 追加字段：irons:[{id, text, anchor, kind, grade, cite?:{name, kind}}]
                      = 锻造当下那几块铁的**快照**，不是 id 引用。
                      为什么存快照不存 id：只存 id 等于把证据链一端拴在可变文件上——
                      data/irons 一改一删，剑就说不清自己拿什么锻的了。谱系必须自带原话。
                      副作用：meta.json 新增同名字段 irons，forge / reforge 落盘时写入。
                      **旧剑（v2 之前锻的）没有这个字段 → 空数组**，前端整节不渲染，不报错。
