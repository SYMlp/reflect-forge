...
⚒ 改：frontmatter `name` 加个人版后缀 `-verify-first`，`description` 末尾追加一句本版特点（四块铁的硬门槛） （依据：「是没验证就说的猜测」）
...
| 光剩机器 ID 的标签 | 节点标签只有 uuid / 容器哈希 / 自动生成 id，人认不出这是什么组件 | 补上人话名称（资源 name / 服务名 / 镜像名），原 id 降为副标签或 tooltip |

⚒ 改：Step 5 自检表新增一行检查项——节点标签只剩机器 id 视为缺陷，自动修复动作是补人话名称、原 id 降为副标签 （依据：「真正病因不是没料，是认不出」）
...
- **全量重生成先拦下仲裁**：layout 级重做等于中途改契约——它会丢弃此前所有轮次攒下的布局微调、端口分配和标签位置。执行前必须停下，向用户报明代价（哪些已调好的东西会没），拿到明确拍板再重生成；不许把「换个方向」当小改顺手做掉。
...
⚒ 改：Review Loop 规则里加一条拦截步骤——全量重生成视为中途改契约，先报明将丢弃的布局微调代价、经用户单点拍板后再动手 （依据：「拦下来发我仲裁」）
...
**外皮不许压内核**：preset 的职责只是把人吸引来看——应用任何 preset 或 `restyle.py` 时，配色、字体、手绘风若与图的信息表达冲突（标签对比度不足、结构线被弱化、为风格统一删减节点或标签），一律内容优先：宁可局部破坏风格一致性，也不牺牲一条边、一个标签的可读性。

⚒ 改：Style Presets 开头加优先级判据——风格应用与信息表达冲突时内容一律优先，禁止为外观牺牲图的内核 （依据：「好看的外表其实就是让人家愿意看看」）
...
最容易跑偏的一种样子（真实踩过的坑型）：没跑 `--version` 就按记忆里的「新版支持」直接发 `--layout` 或喂 `.mmd`，在 ≤ v29 上前者破坏参数解析、后者直接失败——半截导出链废掉才回头查版本。任何「这个 CLI 应该支持 X」的念头，都先当场跑命令验证再落 flag。

⚒ 改：Common Mistakes 补一个反例——凭记忆断定 CLI 能力、不实测版本就发版本门槛 flag 的典型跑偏样子 （依据：「是没验证就说的猜测」）
...
重锻自开源秘籍《drawio-skill》，融入锻造师 4 块铁。改动点：

1. frontmatter —— `name` 加个人版后缀 `-verify-first`，`description` 末尾追加本版特点一句 （依据：「是没验证就说的猜测」）
2. Workflow Step 1（Check deps）—— 加判据：CLI 版本与版本门槛特性（`.mmd` 转换、`--layout`）必须以当场实测 `--version` 输出为准，禁止「可能 / 应该」推定后直接发 flag （依据：「是没验证就说的猜测」）
3. Workflow Step 3（Generate，importer / 实况快照段）—— 加判据：生成图里每个节点必须带人话标识（资源 name / 服务名 / 镜像名），uuid / 哈希只能降为副标签或 tooltip （依据：「真正病因不是没料，是认不出」）
4. Step 5 Self-Check 检查表 —— 补检查行：节点标签只剩机器 id 视为缺陷，自动修复动作是补人话名称 （依据：「真正病因不是没料，是认不出」）
5. Step 6 Review Loop 规则 —— 改步骤：全量重生成视为中途改契约，执行前先拦下、报明将丢弃的布局微调代价、经用户单点拍板再动手 （依据：「拦下来发我仲裁」）
6. Style Presets 开头 —— 加判据：风格应用与信息表达冲突时内容一律优先，禁止为外观牺牲边、标签和结构 （依据：「好看的外表其实就是让人家愿意看看」）
7. Common Mistakes —— 补反例：凭记忆断定 CLI 能力、不实测版本就发版本门槛 flag 的典型跑偏样子 （依据：「是没验证就说的猜测」）
